...
template <typename T> void read(vector<T> &oneD, ll n){f(i,n){ll x; read(x); oneD.push_back(x);}}
...
template <typename T> void write(vector<vector<T>> &rc, T &n, T &m){f(i,n){f(j,m){cout<<rc[i][j]<<" ";}cout<<line;}}
